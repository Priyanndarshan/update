...
const MarketSummary: React.FC<MarketSummaryProps> = () => {
  ...
  const [chartData, setChartData] = useState(generateChartData('AAPL'));
  ...
  const dayRangeLow = (selectedStockData.price - (Math.random() * 5)).toFixed(2);
  const dayRangeHigh = (selectedStockData.price + (Math.random() * 5)).toFixed(2);
  const weekHigh = (selectedStockData.price * 1.2).toFixed(2);
  const weekLow = (selectedStockData.price * 0.8).toFixed(2);
